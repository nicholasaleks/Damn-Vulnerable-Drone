...
if [ "$wifi_simulation" = "Y" ]; then
    ...
    echo "Starting simulation with a virtual Wi-Fi network..."
	...
	pids=($(echo "$output" | grep -oP '^\s*\K[0-9]+(?=\s+\S)'))
        ...
        docker compose logs 2>&1 | tee -a "dvd.log"
    ...
    LOG_FILE="dvd.log"
    {
        echo "Starting simulation assuming drone network connectivity access..."
        echo "[+] Starting Docker Compose..."
    ...
    echo "Invalid input. Please start the script again and enter 'Y' or 'N'."
    exit 1
fi
